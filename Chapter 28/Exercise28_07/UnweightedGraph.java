 ...
 * Date: 04/14/2021
 * 
 * This program was modified to include the getACycle method that returns a list of vertices in a cycle starting from variable u
 */
public class UnweightedGraph<V> implements Graph<V> {
  ...
  public List<Integer> getACycle(){
	...
	while(verticesList.size() > 0){
		int v = verticesList.get(0);
		Stack<Integer> stack = new Stack<Integer>();
		stack.push(v);
		isVisited[v] = true;
		verticesList.remove(new Integer(v));
			...
			if(neighbors.get(x).size() == 0){
				...
				for(int i = 0; i < neighbors.get(x).size(); i++){
					...
					if(!isVisited[e.v]){
						...
						stack.push(e.v);
						isVisited[e.v] = true;
						verticesList.remove(new Integer(e.v));
